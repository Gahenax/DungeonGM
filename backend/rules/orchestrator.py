...
class Orchestrator:
    ...
    def process_action(self, action) -> Dict[str, Any]:
        ...
        try:
            campaign = await self.db.ensure_default_campaign()
            character = await self.db.ensure_default_character(campaign["id"])
            room = await self._ensure_current_room(campaign["id"])

            if action.action_type == "combat":
                result = self._handle_combat(action, room)
                events = [result]
            elif action.action_type == "exploration":
                result = self._handle_exploration(campaign["id"], room)
                room = result["room"]
            ...
            narrative = self._generate_narrative(action, state)
    ...
    async def _ensure_current_room(self, campaign_id: str) -> Dict[str, Any]:
        room = await self.db.get_current_room(campaign_id)
        if room:
            return room

        room = self.generation_engine.initial_room(campaign_id)
        saved_room = await self.db.save_room(room)
        await self.db.set_current_room(campaign_id, saved_room["id"])
        return saved_room

    def _handle_combat(self, action, room) -> Dict[str, Any]:
    ...
    def _handle_exploration(self, campaign_id: str, current_room: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_narrative(self, action, state) -> str:
        room = state.get("room", {})
        prompt = (
